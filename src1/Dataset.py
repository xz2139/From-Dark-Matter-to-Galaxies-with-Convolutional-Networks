...
#             raise ValueError('cat not exist')
    ...
    def __getitem__(self, index):
        ...
        # Select sample
        ID = self.IDs[index]
        d_box = np.load('/oldscratch/xz2139/cosmo_dark/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
        d_box = np.expand_dims(d_box,axis = 0)
        if self.normalize:
            dm_mean = self.dm_mean
            d_box = (d_box - dm_mean) / dm_mean
        if self.cat == 'subcount':
            f_box=np.load('/oldscratch/xz2139/cosmo_full/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
            if not self.reg:
                convert= np.vectorize(self.convert_class) #Convert python function to vector function
                f_box=convert(f_box)
        elif self.cat == 'submass':
            f_box=np.load('/oldscratch/xz2139/cosmo_mass/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
        elif self.cat == 'stellarm':
            dark_box=np.load('/oldscratch/xz2139/cosmo_dark/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
            submass_box=np.load('/oldscratch/xz2139/cosmo_mass/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
            f_box=np.load('/oldscratch/xz2139/cosmo_full/arrays_starm/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
        ...
        if self.vel == True:
            v_box=np.load('/oldscratch/xz2139/cosmo_velo/arrays/'+str(ID[0])+'_'+str(ID[1])+'_'+str(ID[2])+'.npy')
